...
| search_threads | Threads in one course whose title or body contains every query word, case-insensitively. Use when the user asks about a topic rather than a thread number. Ed is paged client-side until limit threads match; accepts the same filters as list_threads. courseId accepts a numeric ID or course code. Then get_thread or read_thread for the body. Cost: several Ed pages for rare words; use limit and since. |
| list_modules | Lesson modules of one course with the number of lessons in each. Use when the user asks which weeks or modules exist, or to pick a module value for list_lessons. courseId accepts a numeric ID or course code. Cost: small. |
| create_thread | Create a thread in one course; posts publicly as the token owner and cannot be removed from here. Use only when the user explicitly asks to post and has approved the title and body. body is Markdown and is converted to Ed's document format. courseId accepts a numeric ID or course code. Cost: one write. |
